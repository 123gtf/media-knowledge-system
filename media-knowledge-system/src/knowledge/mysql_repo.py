...
class MySQLRepository:
    ...
    def upsert_entity(self, entity: Dict[str, Any]) -> Optional[int]:
        """插入或更新实体 (ON DUPLICATE KEY UPDATE)"""
        if not self._connected:
            return None
            ...
            with self._get_connection() as conn:
                ...
                result = conn.execute(
                    ...
                    ),
                    {
                        ...
                        "type": entity.get("type", entity.get("entity_type", "TOPIC")),
                        "aliases": json_module.dumps(entity.get("aliases", [entity.get("name", "")])),
