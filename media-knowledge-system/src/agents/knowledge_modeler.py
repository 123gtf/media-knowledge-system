...
class KnowledgeModelerAgent(BaseAgent):
    ...
    def _create_graph_update_tool(self) -> Tool:
        ...
        @tool(
            name="update_graph",
            description="向Neo4j图谱写入实体节点和关系边，使用MERGE避免重复",
            schema={
                "type": "object",
                "properties": {
        ...
        def update_graph(entities: List[Dict], relations: List[Dict] = None, batch_size: int = 50):
            ...
            if self.graph_store:
                try:
                    ...
                    logger.error(f"图谱实体写入失败: {e}")
                    return {"created_entities": 0, "created_relations": 0, "error": str(e)}

            if self.graph_store and relations:
                try:
                    result = self.graph_store.batch_upsert_relations(relations)
                    created_relations = result.get("created", 0)
                except Exception as e:
                    logger.error(f"图谱关系写入失败: {e}")
                    return {"created_entities": created_entities, "created_relations": 0, "error": str(e)}

            # 同时写入MySQL
            if self.mysql_repo:
                try:
                    for e in entities:
                        self.mysql_repo.upsert_entity(e)
                    if relations:
                        for r in relations:
